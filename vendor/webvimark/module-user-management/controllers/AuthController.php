...
class AuthController extends BaseController
{
	...
	public function actionLogin()
	{
		...
		return $this->renderIsAjax('login', compact('model', 'changelog'));
	}
	...
	public function actionChangeOwnEmail()
	{
		...
		{
			$model->update();
			return $this->renderIsAjax('changeOwnEmailSuccess', compact('model'));
		}

		return $this->renderIsAjax('changeOwnEmail', compact('model'));
	}
    ...
    private function readChangelog() {
        $regex = '/([0-9]+\.[0-9]+.[0-9]+)\s*-\s*([0-9]+\/[0-9]+\/[0-9]+)/';
        $rutaChangelog = Yii::getAlias('@app/.changelog');

        $contador = 0;
        $maxVersiones = 10;
        $changelog = '';
        
        if ($file = fopen($rutaChangelog, 'r')) {
            $matches = [];
