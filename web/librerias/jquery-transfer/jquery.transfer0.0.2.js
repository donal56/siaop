 ...
 ;(function($) {
    ...
    Transfer.prototype.left_group_items_search_handler = function() {
        ...
        $(self.groupItemSearcherId).on("keyup", function () {
			...
			var itemsToHide = $(".transfer-double-group-list-li > div, .transfer-double-group-list-li-ul-li");
			
			//Si esta vacío mostrar todos
            if($(self.groupItemSearcherId).val() == "") {
				for(var j = 0; j < itemsToHide.length; j++) {
					var show = false;
					
					if(itemsToHide.eq(j).is("li")) {
						show = itemsToHide.eq(j).parent("ul").parent("li").children("div").children("input[type=checkbox]").prop("disabled") !== true;
						show = show && !itemsToHide.eq(j).hasClass("selected-hidden");
					}
					else {
						show = true;
					}
					...
					if(show) {
						itemsToHide.eq(j).css('display', 'block');
					}
				}
            }
			else {
				...
				var searchTerm = $(self.groupItemSearcherId).val().trim().toLowerCase();
				searchTerm = searchTerm.replace("ñ", "n");
				...
				for(var j = 0; j < itemsToHide.length; j++) {
					if (!itemsToHide.eq(j).hasClass("selected-hidden")) {
						var termToCompare = itemsToHide.eq(j).text().trim().toLowerCase();
						termToCompare = termToCompare.replace("ñ", "n");
						termToCompare = termToCompare.replace("á", "a");
						termToCompare = termToCompare.replace("é", "e");
						termToCompare = termToCompare.replace("í", "i");
						termToCompare = termToCompare.replace("ó", "o");
						termToCompare = termToCompare.replace("ú", "u");
							
						if(termToCompare.includes(searchTerm)) {
							
							var show = false;
							var el;
						
							if(itemsToHide.eq(j).is("li")) {
								el 		=	itemsToHide.eq(j).parent("ul").parent("li").children("div");
								show 	= 	el.children("input[type=checkbox]").prop("disabled") !== true;
								show 	= 	show && !itemsToHide.eq(j).hasClass("selected-hidden");
							}
							else {
								el		=	itemsToHide.eq(j).parent("li").find("ul li:not(.selected-hidden)")
								show	=	true;
							}
