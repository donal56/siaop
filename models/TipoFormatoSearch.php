<?php

namespace app\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\TipoFormato;

/**
 * TipoFormatoSearch represents the model behind the search form of `app\models\TipoFormato`.
 */
class TipoFormatoSearch extends TipoFormato {
    
    /**
     * {@inheritdoc}
     */
    public function rules() {
        return [
            [['id_tipo_formato', 'id_empresa', 'activo', 'usuario_version'], 'integer'],
            [['tipo_formato', 'fecha_version'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios() {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params) {
        $query = TipoFormato::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id_tipo_formato' => $this->id_tipo_formato,
            'id_empresa' => $this->id_empresa,
            'activo' => $this->activo,
            'fecha_version' => $this->fecha_version,
            'usuario_version' => $this->usuario_version,
        ]);

        $query->andFilterWhere(['like', 'tipo_formato', $this->tipo_formato]);

        return $dataProvider;
    }
}
